...
function ResetPasswordForm() {
  ...
  useEffect(() => {
    if (!token) setError(t("auth.reset.invalid"));
  }, [token, t]);
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm p-8">
          ...
          {success ? (
            <div className="text-center">
              ...
              <h1 className="text-xl font-extrabold text-slate-900 dark:text-white mb-2">{t("auth.reset.success_title")}</h1>
              <p className="text-slate-500 dark:text-slate-400 text-sm">{t("auth.reset.success_body")}</p>
            </div>
          ) : (
            <>
              ...
              {error && !token ? (
                <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 text-sm px-4 py-3 rounded-xl flex items-start gap-2 mb-4">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">{t("auth.reset.label")}</label>
              ...
              <p className="mt-6 text-center text-sm text-slate-500 dark:text-slate-400">
                <Link href="/login" className="text-indigo-600 hover:underline">{t("auth.reset.back")}</Link>
              </p>
            </>
          )}
        </div>
...
export default function ResetPasswordPage() {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center"><div className="text-slate-400 text-sm">…</div></div>}>
      <ResetPasswordForm />
    </Suspense>
  );
}
